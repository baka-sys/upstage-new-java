...
@Service
@Slf4j
public class UpdateEntryDomainJob extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        log.info("执行cron定时任务更换入口域名{}", LocalDateTime.now());
        try {
            handle();
    ...
    private void handle() throws IllegalAccessException {
        ...
        // 定义开始时间和结束时间，每4个小时更换一个
        LocalTime startTime = LocalTime.of(0, 0, 0); // 00:00:00
        LocalTime endTime = LocalTime.of(7, 59, 0); // 08:30:00
        ...
        if (currentTime.isBefore(endTime) && !currentTime.isBefore(startTime)) {
            ...
            // 在这里添加你需要执行的代码
            systemDomainService.updateMainUrlJob();
        }
    }
}
